-- Add OpenAI as a second poll-reconciled video provider.
--
-- This is deliberately corrective: the already-applied video ledger migration
-- remains immutable. Provider changes are allowed only before any provider
-- request is bound, so switching cannot submit the same debit to two providers.
-- The Google webhook binding RPC is intentionally not replaced here.

alter table public.video_generation_jobs
  drop constraint video_generation_jobs_provider_kind_valid;
...
drop index if exists public.video_generation_jobs_google_reconcile_idx;
create index video_generation_jobs_async_reconcile_idx
  ...
  where provider_name in ('google', 'openai')
...
begin
  ...
  if p_user_id is null
     ...
     or p_provider_name not in ('fal', 'google', 'openai')
...
begin
  ...
  if request.provider_request_id is not null
     or request.submitted_at is not null then
    return jsonb_build_object('status', 'already_submitted');
  end if;
  if p_expected_provider_name not in ('fal', 'google')
     or p_new_provider_name not in ('google', 'openai')
     or p_expected_provider_name = p_new_provider_name
     or (
       p_expected_provider_name = 'google'
       and p_new_provider_name <> 'openai'
     ) then
  ...
  update public.video_generation_jobs as ledger
   ...
   where ledger.id = request.id
     ...
     and ledger.submitted_at is null
...
begin
  ...
  for request in
    select ledger.*
      from public.video_generation_jobs as ledger
     where ledger.provider_name in ('google', 'openai')
     ...
     where ledger.status = 'queued'
       ...
       and (
         ...
         or (
           ...
           and ledger.provider_name not in ('google', 'openai')
...
-- Reassert least-privilege ACLs after replacing the security-definer RPCs.
