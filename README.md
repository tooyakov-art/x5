# X5

Native iOS marketing workspace built with SwiftUI. X5 combines AI image tools, voice generation, courses, chat, specialist tasks, and profile/account flows in one native app.

## Status

- **Bundle ID:** `com.x5studio.app`
- **App Store name:** `X5`
- **Version:** 1.1.6 (build 206 release target; build 205 is the last verified TestFlight build)
- **Branch:** `codex/x5-home-width-fix-20260808`
- **Apple Team:** F8LA8PC4U6
- **Stack:** Swift 5.9 + SwiftUI, iOS 16+, no external libraries
- **Backend:** X5 Supabase Edge Functions and private Storage; provider credentials stay server-side
- **Release:** GitHub Actions builds and uploads to TestFlight
- **Current outcome:** YouTube cover generator has six creative modes and defaults to 16:9; Home exposes the existing voice generator
- **Verification:** focused source regressions pass; full regression and build 206 TestFlight verification are pending in this change
- **Remaining:** direct ElevenLabs account/provider configuration is deferred; the current protected fal ElevenLabs path remains active

## What's inside

- Native Home, CourseUP, chats, Hub, profile, search, and portfolio flows
- AI image generation with category-specific briefs, references, and progress recovery
- YouTube covers: entertainment, serious, expert, news, provocative, and minimal modes
- Voice generation with language, voice, stability, speed, playback, and sharing
- Server-side credits, idempotency, private media storage, and signed downloads
- Sign in with Apple and in-app account deletion

## Release safety

- No provider secret is embedded in the iOS binary.
- Voice requests use idempotency and authenticated server functions.
- Generated media is stored privately and delivered through short-lived signed URLs.
- Account deletion remains available inside the app.

## Project layout

```
x5/
├── project.yml                       # XcodeGen spec (project.pbxproj is generated, not committed)
├── ExportOptions.plist               # archive export config (App Store Connect, manual signing)
├── X5/
│   ├── X5App.swift                   # @main entry
│   ├── ContentView.swift             # auth gate
│   ├── X5.entitlements               # Sign in with Apple
│   ├── Info.plist                    # generated by XcodeGen from project.yml
│   ├── Views/
│   │   ├── LoginView.swift
│   │   ├── MainView.swift
│   │   └── ProfileView.swift
│   ├── Services/
│   │   ├── Auth.swift                # ObservableObject managing session state
│   │   ├── SupabaseClient.swift      # plain URLSession REST client (no Supabase SDK)
│   │   └── Captions.swift            # template-based generator
│   └── Assets.xcassets/
│       ├── AppIcon.appiconset/       # AppIcon-1024.png (auto-generated)
│       ├── AccentColor.colorset/     # #00D9A3
│       └── LaunchBackground.colorset/# #0A0A12
├── .github/workflows/
│   └── ios-build.yml                 # macOS-14 runner, archives + uploads to TestFlight
├── supabase/
│   └── 001_delete_own_account.sql    # apply once via Supabase SQL Editor
├── scripts/
│   └── gen-icon.mjs                  # regenerates AppIcon-1024.png
└── SUBMIT.md                         # full submission walkthrough
```

## Build it

You do not need a Mac. Push to `main`, GitHub Actions builds and uploads to TestFlight.

See [`SUBMIT.md`](./SUBMIT.md) — full step-by-step including:
- Bundle ID registration
- App Store Connect entry creation
- Distribution certificate + provisioning profile generation
- App Store Connect API key creation
- GitHub Secrets configuration
- Workflow trigger
- Metadata + review notes

Total active time: ~2.5 hours setup + 30 min per submission. Apple review wait: 1–3 days.
