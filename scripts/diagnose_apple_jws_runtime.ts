...
function safeMessage(error: Error): string {
  return error.message
    .replace(/[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,}/g, "[redacted-jws]")
...
const signedPayload = (await Deno.readTextFile(required("SIGNED_PAYLOAD_PATH")))
  .trim();
const rootPaths = required("APPLE_ROOT_CA_PATHS").split(",").map((path) =>
  path.trim()
).filter(Boolean);
if (rootPaths.length === 0) throw new Error("missing_apple_root_ca_paths");

const roots = await Promise.all(
  rootPaths.map(async (path) => Buffer.from(await Deno.readFile(path))),
);
...
try {
  if (Deno.env.get("PAYLOAD_KIND") === "transaction") {
    ...
    console.log(
      `verified=transaction bundle_match=${payload.bundleId === BUNDLE_ID} environment=${payload.environment ?? "-"}`,
    );
  } else {
    const payload = await verifier.verifyAndDecodeNotification(signedPayload);
    console.log(
      `verified=notification bundle_match=${payload.data?.bundleId === BUNDLE_ID} environment=${payload.data?.environment ?? "-"}`,
    );
  }
} catch (error) {
  report(error);
