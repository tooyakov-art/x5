    ...
    var body: some View {
        ...
        .fullScreenCover(isPresented: $showingPostViewer) {
            PortfolioInstagramViewer(
                ...
                onReplaceMedia: { item, data, mediaType, mime, ext in
                    guard let token = auth.accessToken else { return nil }
                    return await service.replaceMedia(data: data, type: mediaType, mime: mime, ext: ext, itemId: item.id, userId: userId, accessToken: token)
                },
                isPinned: { item in
...
private struct PortfolioInstagramViewer: View {
    ...
    let onReplaceMedia: (PortfolioItem, Data, String, String, String) async -> PortfolioItem?
                    ...
                    LazyVStack(spacing: 0) {
                        ForEach(items) { item in
                            PortfolioInstagramPostPage(
                                ...
                                onReplaceMedia: { data, mediaType, mime, ext in await onReplaceMedia(item, data, mediaType, mime, ext) },
...
private struct PortfolioInstagramPostPage: View {
    ...
    let onReplaceMedia: (Data, String, String, String) async -> PortfolioItem?
    let onLoadLike: () async -> PortfolioLikeState
    let onSetLiked: (Bool) async -> Bool
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingEdit) {
            EditPortfolioItemView(item: item, onSave: onUpdateDetails, onReplaceMedia: onReplaceMedia)
                .preferredColorScheme(.dark)
        }
    }
...
private struct EditPortfolioItemView: View {
    ...
    init(
        item: PortfolioItem,
        onSave: @escaping (String?, String?) async -> PortfolioItem?,
        onReplaceMedia: @escaping (Data, String, String, String) async -> PortfolioItem?
    ) {
        self.item = item
        self.onSave = onSave
        self.onReplaceMedia = onReplaceMedia
            ...
            Form {
                Section("Медиа") {
                    PhotosPicker(selection: $mediaItem, matching: .any(of: [.images, .videos])) {
                        Label(mediaData == nil ? "Заменить фото или видео" : "Новое медиа выбрано", systemImage: mediaType == "video" ? "play.rectangle.fill" : "photo.on.rectangle")
                    }
                    .onChange(of: mediaItem) { newValue in
                        Task { await loadMedia(newValue) }
                    }
                    if mediaData != nil {
                        Text("Медиа заменится после сохранения.")
                            .font(.footnote)
    ...
    private func save() async {
        ...
        let details = await onSave(cleanTitle.isEmpty ? nil : cleanTitle,
                                   cleanDescription.isEmpty ? nil : cleanDescription)
        let media = mediaData == nil ? item : await onReplaceMedia(mediaData!, mediaType, mime, ext)
        if details != nil && media != nil {
            X5Feedback.success()
            dismiss()
        } else {
            X5Feedback.error()
            ...
            Form {
                Section {
                    VStack(alignment: .leading, spacing: 6) {
                        Text("Новый кейс")
                            .font(.headline)
                        Text("Добавь работу с медиа, названием и описанием. Потом ее можно редактировать или удалить из меню поста.")
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                    }
                    PhotosPicker(selection: $mediaItem, matching: .any(of: [.images, .videos])) {
